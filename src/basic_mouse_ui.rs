...
impl<T: std::cmp::Eq + std::hash::Hash> VerticalMenu<T> {
    pub fn new(view_up: i64, view_down: i64, text_pixel_size: [f64;2], selected_background_rgba: [f32;4])->Self{
        return VerticalMenu{
            selected: 0,
            view_up,
            view_down,
    ...
    pub fn render(
        ...
        for i in self.selected-self.view_up..self.selected+self.view_down{
            let pythons_index = ((i.abs()+i)/2)% i_length +(i_length -((i.abs()-i)/2)% i_length)% i_length;
